...
const fetchCategories = async () => {
  ...
  try {
    ...
    // Check if response data and categories exist
    if (response.data && Array.isArray(response.data.categories)) {
      setCategories(response.data.categories); // Update categories state with fetched data
    } else {
      throw new Error("Invalid response structure: 'categories' field is missing or not an array.");
    }
  } catch (error) {
    // Handle errors
    setError(error);
